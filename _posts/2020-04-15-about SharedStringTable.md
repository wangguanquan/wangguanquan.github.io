...
SharedStringTable是微软在BIFF8版本中引入的概念，其目地是整个Workbook共享一个字符串区域，所有的Worksheet只需要保存字符串下标即可，读取文档时根据下标值从SharedStringTable中读取相应的值。
...
SharedStringTable的操作顺序像这样:写Worksheet时先从SharedStringTable中查找字符串，如果找到则将下标当做值写入文件，如果未找到则将字符串追加到SharedStringTable最后，再将当前下标写入文件，读取时通过下标拿回SharedStringTable中相应的字符串。简单的理解可以将SharedStringTable看成是超市门口的储物柜，进超市前你需要将背包给交给管理人员，管理人员会给你换一个条形码或手环，逛完超市出来通过条形码或手环去取回你的背。

下面我们使用案例描述SST是如何运作的*为了方便描述，以下将SharedStringTable简写为SST*
...
读取字符串时只需要使用val值从SST取相应下标的字符串即可。
...
与SharedString相对应的方式是`innerStr`，从字面意思不难理解，它将字符串值直接写到每个Worksheet中，所以被称为inner模式。在各Worksheet.xls中大概像下面这样
...
`is`是`innerStr`的简写，此标签与sharedStrings.xml中的`si`标签对应。
...
首先想到的是使用Array实现，它是顺序的且扩容不影响原有顺序，理论上要实现一个ArraySST很简单，只需要简单封装一个字符串数组即可，下面我们先实现一个简单的SST。
...
既然我们解决了时间复杂度问题，似乎应该从空间复杂度来重新审视这种实现。它似乎比较占内存...不用怀疑它真的非常耗内存，在[EEC vs easyexcel(三)](https://www.ttzero.org/excel/2020/04/20/eec-vs-easyexcel-3.html#3-xls%E6%A0%BC%E5%BC%8F%E8%AF%BB%E5%86%99)性能测试中，读取一个978MB大小的xls文件POI需要用到超过5G的内存，读文件使用的内存是文件大小的5倍...这大概不是我们想要的结果，继续优化吧。

#### 改造第2版

这一版我们要解决字符串数量超大(超过百万，千万)时，无法将所有字符串全部放入内存应该如何处理？Trie树？B+树？
