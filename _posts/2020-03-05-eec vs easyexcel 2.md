...
第一个是easyexcel导出100w数据，我们看到从21:34:44.412开始到21:36:17.902写完100w数据，再到21:37:09.910完成数据压缩，写数据用时93.49秒，压缩用时52.008秒
而eec写100w用时27.43秒(实际比这个大一点，因为eec使用pull方式拉数据，我们无法知道最后一批数据完成的实际时间)，压缩用了39.373秒(实际比这个小一点)，写数据的速度远超easyexcel，我们进一步列出所有写数据和压缩时间
...
上图可以发现eec写数据的速度远大于easyexcel，压缩速度略快于后者(eec的压缩比为5，文件可能较easyexcel大)。


### 2. Office默认的字符串Shared模式读性能对比
